...
const technologyValidationSchema = z
    ...
    .max(TECHNOLOGY_MAX_LENGTH, `Technology name cannot exceed ${TECHNOLOGY_MAX_LENGTH} characters`);
const technologiesValidationSchema = z
    .array(technologyValidationSchema)
    .max(MAX_TECHNOLOGIES, `You can add a maximum of ${MAX_TECHNOLOGIES} technologies`)
    .refine((technologies) => new Set(technologies.map((technology) => technology.toLowerCase())).size === technologies.length, {
...
const createHeroBodySchema = z
    ...
    subtitle: z
        ...
        .max(SUBTITLE_MAX_LENGTH, `Subtitle cannot exceed ${SUBTITLE_MAX_LENGTH} characters`),
    description: z
        .string()
        .trim()
    ...
    ctaButtonText: z
        .string()
        .trim()
        .max(CTA_TEXT_MAX_LENGTH, `CTA button text cannot exceed ${CTA_TEXT_MAX_LENGTH} characters`)
        .optional(),
    ctaButtonLink: z
        .url("CTA button link must be a valid URL")
        .trim()
        .max(CTA_LINK_MAX_LENGTH, `CTA button link cannot exceed ${CTA_LINK_MAX_LENGTH} characters`)
        .optional(),
...
//# sourceMappingURL=hero.validation.js.map