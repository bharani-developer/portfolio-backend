...
import { CERTIFICATION_MESSAGE, CERTIFICATION_SEARCHABLE_FIELDS, } from "./certifications.constant.js";
import { Certification } from "./certifications.model.js";
const baseService = new BaseCrudService(Certification, [
...
const getCertifications = async (query) => {
...
const getCertificationsBySkill = async (skill) => {
    return Certification.find({
        skills: {
            $in: [skill],
        },
...
const getCertificationsByIssuer = async (issuer) => {
    return Certification.find({
        issuer,
        isActive: true,
    })
